...
- 行李清单从 Notion 的 `To bring list` 导入：Rylee 管理背包1，Nobby 管理背包2，每人均有“随身”和“背包里”分区。共同用品采用“携带者装包、另一人复核”，两边同名物品自动提示重复；登录成员可勾选、长按拖动同分区物品排序、转交、删除及新增物品，并可切换为只看待装，变更实时同步。
