...
- 「今日」入口汇集当天城市、住宿、当前 / 下一事项、重要交通的离开酒店时间与当日路线；不在旅行日期时可切换到任一日预览。
- 手机日历为逐日卡片，桌面仍使用原表格；交通与住宿页显示时间、状态、地图入口和已配置的转场提醒。
- 今日路线按待办顺序连接可定位的收藏地点，仅表示行程先后，不是道路导航。待办、地图标记和收藏卡片可互相跳转。
...
- 六地酒店与地图查询入口。
- [Google Maps 收藏地点原清单](https://maps.app.goo.gl/JQUJwBDUwqy2TfQP9)；[最新清单](https://maps.app.goo.gl/y1Kww3mHPbMZbS1c8)。最新清单已导入 110 个地点（其中两处现代主义建筑由网站先行收录，与清单中的同地点合并）；每个地点均展示用途介绍、建议停留、行前提示、住宿距离、当前安排和原备注。地图入口使用原收藏名称、所属城市与国家打开具体地点，不再把裸经纬度作为外链名称。
- 地图标记按密度聚合，并与地点卡片双向定位；登录成员可从地点卡片直接安排到某日的日历或待办。
- 收藏地点支持“必去、顺路、备选、已放弃”四种协作状态、一键置顶和快捷筛选；状态与置顶顺序由协作后台同步保存。
- 行李清单从 Notion 的 `To bring list` 导入：Rylee 管理背包1，Nobby 管理背包2，每人均有“随身”和“背包里”分区。共同用品置顶汇总并标明提供人，采用“携带者装包、另一人复核”，两边同名物品自动提示重复；登录成员可修改单件物品名称、勾选、在“随身”和“背包里”之间转换、长按拖动同分区物品排序、转交、删除及新增物品，并可切换为只看待装，变更实时同步。
...
交通时间、车次与确认状态以 `legs` 为准；新待办可使用 `legId` 引用交通，旧待办仍可按同日车次识别。新收藏关联使用 `placeIds`，旧地图名称保留并作兼容匹配。`days` 保留每日摘要，酒店显示名称继续以城市资料为准。可选的 `leaveHotelTime`、`arriveTerminalTime`、`originMapQuery`、`destinationMapQuery`、`hotelMapQuery` 和 `breakfast` 字段无需数据库迁移；旧云端字段缺失时采用默认或现有资料回退。行程校验仅在浏览器控制台提示，不中断浏览。
